...
class Reaction
{
	...
	public function send_reaction($user, $pass, $token, $r_male, $r_female, $max_status)
	{
		...
		$get_user_id = 'https://graph.facebook.com/me?access_token=' . $token;

		$u_data = file_get_contents($get_user_id);

		$user_id_json = json_decode($u_data, true);
		$user_id = $user_id_json['id'];
		$user_gen = $this->gender_check($user_id, $token);
